...
import { Users, Plus, Trash2, Search, User, Pencil } from "lucide-react";
...
export default function DashboardCandidats() {
  ...
  const updateMutation = trpc.candidat.update?.useMutation({
    onSuccess: () => utils.candidat.list.invalidate(),
  });
  ...
  const handleCreate = async () => {
    if (
      ...
      alert(
        "Veuillez remplir tous les champs obligatoires et choisir une école existante."
      );
      return;
    }
  ...
  const handleEditOpen = (candidat: any) => {
    setEditingId(candidat.idInscription);
    setEditForm({
      nom: candidat.nom,
      prenom: candidat.prenom,
      sexe: candidat.sexe,
      dateNaissance: candidat.dateNaissance || "",
      idSerie: candidat.idSerie,
      idEcole: candidat.idEcole,
    });
    setEditOpen(true);
  };
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-lg">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    value={form.idSerie}
                    onChange={e =>
                      setForm({ ...form, idSerie: Number(e.target.value) })
                    }
                    className="w-full h-10 px-3 rounded-md border border-input bg-background text-sm"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <select
                    value={form.idEcole}
                    onChange={e =>
                      setForm({ ...form, idEcole: Number(e.target.value) })
                    }
                    className="w-full h-10 px-3 rounded-md border border-input bg-background text-sm"
                  >
                    <option value={0}>Choisir...</option>
                    {ecolesQuery.data?.map(e => (
                      <option key={e.idEcole} value={e.idEcole}>
                        {e.nom}
                      </option>
                    ))}
                  </select>
              ...
              <Button
                ...
                className="w-full bg-[#1E8B4C] hover:bg-[#167a3f] mt-4"
              >
                Ajouter
              </Button>
            </DialogContent>
            ...
            <DialogContent className="max-w-lg">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4">
                ...
                <div className="space-y-2">
                  <Label>Ecole *</Label>
                  <select
                    value={editForm.idEcole}
                    onChange={e =>
                      setEditForm({
                        ...editForm,
                        idEcole: Number(e.target.value),
                      })
                    }
                    className="w-full h-10 px-3 rounded-md border border-input bg-background text-sm"
                  >
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right flex gap-2 justify-end">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        deleteMutation.mutate({ id: c.idInscription })
                      }
